...
Three close-call cells where the gap between the top two backends is less than 3\% receive targeted five-repetition anchoring on Modal H100 (Section~\ref{sec:modal-anchors}): SafeTensors Qwen3-14B (sync vs.\ default), ServerlessLLM Qwen3-14B (sync vs.\ default), and ServerlessLLM Qwen3-4B (async vs.\ default). All three show overlapping ranges, confirming that these differences are not statistically separable.

\textbf{External baseline.} In addition to internal backend comparisons, a reference baseline (\texttt{hf-native}) wraps the upstream SafeTensors crate's loading path (\texttt{std::fs::read} + \texttt{SafeTensors::deserialize}) with no Tensora planning, chunking, or backend selection. This represents what a user obtains from the SafeTensors library alone and provides an external reference point for assessing Tensora's practical value (Section~\ref{sec:hf-native-results}).
...
A fourth environment uses Modal's serverless GPU infrastructure for three experiments: held-out model validation (Section~\ref{sec:held-out}), full-matrix anchoring with targeted close-call analysis (Section~\ref{sec:modal-anchors}), and \texttt{hf-native} external baseline measurement (Section~\ref{sec:hf-native-results}). Each Modal container provisions an H100 GPU with 32\,GB RAM and 512\,GB ephemeral NVMe. The container runs gVisor (a userspace kernel), which blocks \texttt{io\_uring} with \texttt{ENOSYS}; only sync, async, and default backends are evaluated. Models are downloaded from the HuggingFace CDN on each cold run, and page-cache eviction uses \texttt{posix\_fadvise(DONTNEED)} rather than \texttt{drop\_caches}. These methodological differences mean that absolute timings are not directly comparable to the primary H100 results; the Modal experiments test regime transfer and provide statistical support, not identical millisecond reproduction.
