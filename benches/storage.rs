...
fn bench_sync_reader_load(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("load", &slug), |b| {
        b.iter(|| {
            let data = sync::File::open(black_box(shard.as_path()))
                .unwrap()
                .read_all()
                .unwrap();
            black_box(data.len())
...
fn bench_tokio_storage_load(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("load", &slug), |b| {
        b.to_async(&rt).iter(|| {
            ...
            async move {
                let data = fastio_tokio::File::open(black_box(path.as_path()))
                    .await
                    .unwrap()
                    .read_all()
                    .await
...
fn bench_io_uring_reader_load(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("load", &slug), |b| {
        b.iter(|| {
            let data = uring::File::open(black_box(shard.as_path()))
                .unwrap()
                .read_all()
                .unwrap();
            black_box(data.len())
        ...
        b.to_async(&rt).iter(|| {
            ...
            async move {
                ...
                for path in black_box(&paths) {
                    total_len += fastio_tokio::File::open(path.as_path())
                        .await
                        .unwrap()
                        .read_all()
                        .await
...
fn bench_mmap_open_touch(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("open_touch", &slug), |b| {
        b.iter(|| {
            let mmap = mmap::File::open(black_box(shard.as_path()))
                .unwrap()
                .map()
                .unwrap();
