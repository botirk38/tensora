...
fn bench_mmap(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("load", &slug), |b| {
        b.iter(|| {
            let model = ServerlessLLMCheckpoint::open(black_box(&dir_str)).unwrap();
            let bytes: usize = model
                .tensor_names()
...
fn bench_tensor_sequential(c: &mut Criterion) {
    ...
    let model = ServerlessLLMCheckpoint::load(&dir_str, tensora::Backend::Sync).unwrap();
...
fn bench_mmap_tensor_sequential(c: &mut Criterion) {
    ...
    let mmap_model = ServerlessLLMCheckpoint::open(&dir_str).unwrap();
