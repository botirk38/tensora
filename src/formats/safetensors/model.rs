...
fn choose_load_backend(stats: &LoadStats) -> LoadBackend {
    #[cfg(target_os = "linux")]
    {
        let io_uring_avail = backends::io_uring::availability().is_available();
        choose_load_backend_with_availability(stats, io_uring_avail)
    }
...
#[cfg(target_os = "linux")]
fn choose_load_backend_with_availability(
    stats: &LoadStats,
    ...
    if score >= 48.0 && io_uring_available {
...
mod tests {
    use super::*;
    #[cfg(target_os = "linux")]
    use crate::backends::{BackendAvailability, BackendUnavailableReason};
    ...
    fn selector_uses_async_for_large_safetensors_when_io_uring_unavailable() {
        ...
        assert_eq!(
            choose_load_backend_with_availability(&stats, false),
            LoadBackend::TokioAsync
        );
    }
