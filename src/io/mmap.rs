...
/// demand.  It does **not** implement `BlockingIo` or `BlockingIo`.
///
/// ```rust,ignore
/// use tensora::io::mmap::Mmap;
/// use tensora::io::{MmapIo};
/// use std::path::Path;
///
/// let engine = Mmap::new();
...
mod tests {
    use super::*;
    use crate::io::{ByteRange, Io, MmapIo};
    ...
    fn availability_is_available() {
        assert!(Mmap::availability().is_available());
    }
