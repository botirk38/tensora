...
def test_load_serverlessllm_sync(serverlessllm_dir, hidden_dim):
    d = load_serverlessllm(serverlessllm_dir, backend="sync")
    assert "wte" in d and "wpe" in d
    assert d["wte"].shape == (1024, hidden_dim)
    assert d["wpe"].shape == (128, hidden_dim)
...
def test_load_serverlessllm_async(serverlessllm_dir, hidden_dim):
    d = load_serverlessllm(serverlessllm_dir, backend="async")
    assert "wte" in d and "wpe" in d
    assert d["wte"].shape == (1024, hidden_dim)
...
def test_load_keys_parity(serverlessllm_dir):
    ...
    keys_sync = set(load_serverlessllm(serverlessllm_dir, backend="sync").keys())
    keys_async = set(load_serverlessllm(serverlessllm_dir, backend="async").keys())
...
def test_load_shapes_parity(serverlessllm_dir):
    ...
    d_sync = load_serverlessllm(serverlessllm_dir, backend="sync")
    d_async = load_serverlessllm(serverlessllm_dir, backend="async")
    assert (
