...
def test_embedding_lookup_sync(safetensors_path):
...
def test_embedding_lookup_mmap(safetensors_path):
...
@pytest.mark.asyncio
async def test_embedding_lookup_async(safetensors_path):
...
def test_attention_computation_sync(safetensors_path):
...
def test_attention_computation_mmap(safetensors_path):
...
@pytest.mark.asyncio
async def test_attention_computation_async(safetensors_path):
...
def test_full_layer_forward_sync(safetensors_path):
    """Test full layer forward with sync-loaded weights."""
    output = _full_layer_forward(load_safetensors_sync(safetensors_path))
    assert output.shape == (2, 5, 768)
    assert not torch.isnan(output).any()
    assert not torch.isinf(output).any()


def test_full_layer_forward_mmap(safetensors_path):
    """Test full layer forward with mmap-loaded weights."""
    output = _full_layer_forward(load_safetensors_mmap(safetensors_path))
    assert output.shape == (2, 5, 768)
    assert not torch.isnan(output).any()
    assert not torch.isinf(output).any()
...
@pytest.mark.asyncio
async def test_full_layer_forward_async(safetensors_path):
    """Test full layer forward with async-loaded weights."""
    output = _full_layer_forward(await load_safetensors(safetensors_path))
    assert output.shape == (2, 5, 768)
    assert not torch.isnan(output).any()
    assert not torch.isinf(output).any()
...
def test_gradient_flow_mmap(safetensors_path):
    """Test gradient flow with mmap-loaded weights."""
    x_grad, c_attn_grad, c_proj_grad = _gradient_test(
        load_safetensors_mmap(safetensors_path)
    )
    assert x_grad.shape == (1, 10, 768)
    assert c_attn_grad.shape == (768, 2304)
    assert c_proj_grad.shape == (768, 768)
...
@pytest.mark.asyncio
async def test_gradient_flow_async(safetensors_path):
    """Test gradient flow with async-loaded weights."""
    x_grad, c_attn_grad, c_proj_grad = _gradient_test(
        await load_safetensors(safetensors_path)
    )
    assert x_grad.shape == (1, 10, 768)
    assert c_attn_grad.shape == (768, 2304)
    assert c_proj_grad.shape == (768, 768)
